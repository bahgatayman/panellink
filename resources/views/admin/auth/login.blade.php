...
<head>
    ...
    <title>{{ __('app.auth.linkspace') }} - {{ __('app.auth.admin_login') }}</title>
...
<body class="min-h-screen bg-gradient-to-br from-red-50 to-red-100 flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
        <div class="flex justify-center mb-4">
            <img src="/logo.png" alt="Link Space Panel" class="h-20 w-auto">
        </div>
        <h1 class="text-2xl font-bold text-gray-900 text-center mb-2">{{ __('app.auth.linkspace_admin') }}</h1>
        <p class="text-sm text-gray-500 text-center mb-8">{{ __('app.auth.admin_panel') }}</p>
