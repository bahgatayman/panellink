...
@section('title', __('app.error.429_title'))
...
@section('content')
    ...
    <div class="text-center">
        <span class="inline-block text-xs font-mono font-semibold tracking-wider text-amber-600 bg-amber-50 px-2.5 py-1 rounded-full mb-5">
            {{ __('app.error.error_code', ['code' => '429']) }}
        </span>
        ...
        <h1 class="text-xl font-bold text-surface-900 mb-2">{{ __('app.error.429_heading') }}</h1>
        <p class="text-sm text-surface-500 mb-8">{{ __('app.error.429_message') }}</p>
