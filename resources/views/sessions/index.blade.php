...
@section('content')
    ...
    @if ($error)
        <div class="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-4">
            MikroTik connection error: {{ $error }}<br>
            <a href="/settings" class="underline font-medium">Check your MikroTik connection in Settings</a>
        </div>
    @endif
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-gray-50 text-gray-500 uppercase text-xs tracking-wider">
                    <tr>
                        ...
                        <th class="px-4 py-3">Phone / Username</th>
                ...
                <tbody class="divide-y">
                    @foreach ($sessions as $session)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3">
                                @if ($session['user_id'])
                                    <a href="/users/{{ $session['user_id'] }}" class="text-blue-600 hover:underline text-sm font-medium">View User</a>
                                @else
    ...
    @elseif (!$error)
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            <p class="text-gray-500 text-lg">No active sessions right now.</p>
        </div>
    @endif
@endsection
